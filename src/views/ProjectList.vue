...
function toggleProject(id) { expandedProjects.value[id] = !expandedProjects.value[id]; }

const editDialog = ref(false);
const editingProject = ref({ id: null, name: "", description: "", status: "active", startDate: null, endDate: null });
...
async function updateProject() {
  await ProjectServices.updateProject(editingProject.value.id, editingProject.value)
    .then(() => { showSnackbar("green", "Project updated successfully!"); editDialog.value = false; })
    .catch((error) => { showSnackbar("error", error.response?.data?.message || "Failed to update project"); });
  await getProjects();
}

function getInitials(u) { return (u.firstName[0] + u.lastName[0]).toUpperCase(); }
function addMember(u) { selectedMembers.value.push({ user: u, role: "member" }); memberSearch.value = ""; }
function removeMember(i) { selectedMembers.value.splice(i, 1); }
    ...
    .then(async (response) => {
      ...
      if (project.value.repoUrl) {
        await RepoServices.addRepo({
          name: project.value.repoUrl.split("/").pop(),
    ...
    <div id="body">
          ...
          <v-card-text v-show="expandedProjects[p.id]">
            ...
            <v-row class="mb-2" align="center">
              <v-col class="pl-6" cols="6">
                ...
                <v-row>{{ p.startDate ? new Date(p.startDate).toLocaleDateString() : '—' }}</v-row>
              </v-col>
              <v-col class="pl-6" cols="6">
                <v-row class="subheader">END DATE</v-row>
                <v-row>{{ p.endDate ? new Date(p.endDate).toLocaleDateString() : '—' }}</v-row>
              </v-col>
            </v-row>
            <v-row class="mb-2" v-if="user && user.role !== 'member'">
              <v-col cols="2">
                <button @click.stop="openEdit(p)" class="editButtonStyle">Edit</button>
              </v-col>
              <v-col cols="2">
                <button @click.stop="deleteProject(p.id)" class="deleteButtonStyle">Delete</button>
              </v-col>
            </v-row>
          </v-card-text>
        </v-expand-transition>
      ...
      <v-dialog v-model="dialog" max-width="550">
        <v-card class="rounded-xl pa-2">
          <v-card-title class="d-flex justify-space-between align-center px-4 pt-4 pb-2">
            ...
            <v-btn icon variant="text" @click="dialog = false; resetProject()">
          ...
          <v-card-text class="px-4">
            ...
            <v-text-field
              ...
              placeholder="e.g. SEV Final Project" bg-color="grey-lighten-4"
            ...
            <v-textarea
              ...
              variant="outlined" density="comfortable" rounded="lg"
              placeholder="What does this project accomplish?" bg-color="grey-lighten-4"
              rows="3" class="mb-1" hide-details
            ></v-textarea>
            ...
            <v-select
              ...
              variant="outlined" density="comfortable" rounded="lg"
              bg-color="grey-lighten-4" class="mb-1" hide-details
            ></v-select>
            ...
            <v-row class="mt-2" no-gutters>
              <v-col cols="6" class="pr-2">
                ...
                <v-text-field
                  ...
                  variant="outlined" density="comfortable" rounded="lg"
                  bg-color="grey-lighten-4" hide-details
                ></v-text-field>
              </v-col>
            ...
            <v-text-field
              ...
              variant="outlined" density="comfortable" rounded="lg"
              placeholder="org/repo-name" bg-color="grey-lighten-4"
              class="mb-1" hide-details
            ></v-text-field>

            <div class="form-label mt-3 d-flex justify-space-between align-center">
              <span>MEMBERS</span>
              <span v-if="selectedMembers.length" class="text-caption text-grey-darken-1">
                {{ selectedMembers.length }} member{{ selectedMembers.length > 1 ? 's' : '' }} added
              </span>
            </div>

            <div v-for="(m, i) in selectedMembers" :key="m.user.id"
              class="d-flex align-center mb-2 pa-2 rounded-lg" style="background:#f5f5f5;">
              ...
              <v-select v-model="m.role" :items="['lead', 'member']"
                variant="outlined" density="compact" rounded="lg"
                hide-details style="max-width:145px;" class="mr-2">
              </v-select>
            ...
            <v-text-field v-model="memberSearch"
              variant="outlined" density="comfortable" rounded="lg"
              placeholder="Search by name or email..." bg-color="grey-lighten-4"
              class="mb-1" hide-details>
            </v-text-field>

            <v-card v-if="filteredUsers.length" class="mt-1 rounded-lg" elevation="3">
              <v-list density="compact">
                <v-list-item v-for="u in filteredUsers" :key="u.id" @click="addMember(u)" style="cursor:pointer;">
                  <template #prepend>
          ...
          <v-card-actions class="px-4 pb-4 justify-end">
            <v-btn variant="text" @click="dialog = false; resetProject()">Cancel</v-btn>
            <v-btn style="background-color: #9B7D8C;" variant="flat" class="text-white rounded-lg px-6" @click="createProject">
              Create Project
            </v-btn>
          </v-card-actions>
        </v-card>
      ...
      <v-dialog v-model="editDialog" max-width="550">
        <v-card class="rounded-xl pa-2">
          ...
          <v-card-text class="px-4">
            <div class="form-label">PROJECT NAME</div>
            <v-text-field v-model="editingProject.name"
              variant="outlined" density="comfortable" rounded="lg"
              bg-color="grey-lighten-4" class="mb-1" hide-details>
            </v-text-field>

            <div class="form-label mt-3">DESCRIPTION</div>
            <v-textarea v-model="editingProject.description"
              variant="outlined" density="comfortable" rounded="lg"
              bg-color="grey-lighten-4" rows="3" class="mb-1" hide-details>
            </v-textarea>

            <div class="form-label mt-3">STATUS</div>
            <v-select v-model="editingProject.status" :items="['active', 'inactive', 'completed']"
              variant="outlined" density="comfortable" rounded="lg"
              bg-color="grey-lighten-4" class="mb-1" hide-details>
            </v-select>
            ...
            <v-row class="mt-2" no-gutters>
              <v-col cols="6" class="pr-2">
                <div class="form-label">START DATE</div>
                <v-text-field v-model="editingProject.startDate" type="date"
                  variant="outlined" density="comfortable" rounded="lg"
                  bg-color="grey-lighten-4" hide-details>
                </v-text-field>
              </v-col>
              <v-col cols="6" class="pl-2">
                <div class="form-label">END DATE</div>
                <v-text-field v-model="editingProject.endDate" type="date"
                  variant="outlined" density="comfortable" rounded="lg"
                  bg-color="grey-lighten-4" hide-details>
                </v-text-field>
              </v-col>
          ...
          <v-card-actions class="px-4 pb-4 justify-end">
            ...
            <v-btn style="background-color: #9B7D8C;" variant="flat" class="text-white rounded-lg px-6" @click="updateProject">
              Save Changes
            </v-btn>
          </v-card-actions>
        </v-card>
